    ...
    <StudentLayout>
        <div class="flex flex-col lg:flex-row gap-12 h-[calc(100vh-160px)]">
            ...
            <div class="flex-1 flex flex-col gap-8 min-w-0">
                ...
                <!-- Lesson Info -->
                <div class="bg-white border border-indigo-100/20 rounded-[32px] p-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
                    <div class="max-w-xl">
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            <span v-if="currentLesson?.completed" class="px-3 py-1 bg-teal-50 text-teal-600 text-[10px] font-bold uppercase tracking-widest rounded-full flex items-center gap-1">
                                <CheckCircle2 class="w-3 h-3" /> Completed
                            </span>
                        </div>
                        <h1 class="text-3xl font-heading font-extrabold tracking-tight leading-tight text-stone-800">{{ currentLesson?.title || course.title }}</h1>
                        <p class="text-stone-500 mt-4 font-medium flex items-center gap-2">
                            <User class="w-4 h-4" />
                            Instructor: <span class="text-stone-800 font-bold">{{ course.instructor }}</span>
                        </p>
                    </div>
                        ...
                        <button
                            ...
                            :class="[
                                currentLesson.completed
                                    ? 'bg-teal-50 border-teal-200 text-teal-600 hover:bg-red-50 hover:border-red-200 hover:text-red-500'
                                    : 'bg-indigo-50 border-indigo-200 text-indigo-500 hover:bg-indigo-500 hover:text-white hover:border-indigo-500',
            ...
            <div class="lg:w-[400px] flex flex-col h-full min-w-0">
                <div class="bg-white border border-indigo-100/20 rounded-[40px] flex flex-col overflow-hidden shadow-sm h-full ring-1 ring-indigo-500/[0.02]">
                    <div class="p-8 border-b border-stone-50 bg-stone-50/30">
                        <div class="flex items-center justify-between">
                            <h2 class="text-xl font-heading font-extrabold tracking-tight text-stone-800">Course Content</h2>
                            <span class="text-[10px] font-bold uppercase tracking-[0.2em] text-stone-400">{{ course.lessons.length }} Items</span>
                        </div>
                        <!-- Overall Progress -->
                        <div class="mt-6">
                            ...
                            <div class="flex items-center justify-between mt-3">
                                <p class="text-[10px] font-bold uppercase tracking-widest text-stone-400">Course Progress</p>
                                <p class="text-[10px] font-bold uppercase tracking-widest" :class="progress === 100 ? 'text-teal-500' : 'text-indigo-500'">{{ progress }}%</p>
                            </div>
                        </div>
                    </div>
                        ...
                        <button
                            ...
                            :class="[
                                currentLesson?.id === lesson.id
                                    ...
                                    : lesson.completed
                                        ? 'bg-teal-50/50 text-teal-700 hover:bg-teal-50'
                                        : 'hover:bg-indigo-50 text-stone-500 hover:text-indigo-500',
                    ...
                    <!-- Bottom Nav -->
                    <div class="p-6 border-t border-stone-50 flex items-center justify-between">
                         <button @click="prevLesson" class="p-4 hover:bg-indigo-50 rounded-2xl transition-colors text-stone-400 hover:text-indigo-500" :disabled="currentLessonIndex <= 0">
                         ...
                         <button @click="toggleComplete(currentLesson?.id)" :disabled="toggling || !currentLesson"
                            ...
                            :class="currentLesson?.completed ? 'bg-teal-50 text-teal-600' : 'bg-indigo-50 text-indigo-500 hover:bg-indigo-500 hover:text-white'">
