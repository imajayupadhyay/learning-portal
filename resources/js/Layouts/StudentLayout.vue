<template>
    <div class="min-h-screen bg-[#FAFAF9] text-stone-900 antialiased selection:bg-indigo-500 selection:text-white relative overflow-x-hidden font-body">
        <!-- Abstract Background Mesh -->
        <div class="fixed inset-0 z-0 overflow-hidden pointer-events-none">
            <div class="absolute -top-[10%] -left-[10%] w-[40%] h-[40%] rounded-full bg-indigo-50/50 blur-[120px]"></div>
            <div class="absolute top-[20%] -right-[5%] w-[30%] h-[30%] rounded-full bg-teal-50/40 blur-[100px]"></div>
            <div class="absolute bottom-[10%] left-[30%] w-[25%] h-[25%] rounded-full bg-indigo-50/30 blur-[100px]"></div>
        </div>

        <!-- Sidebar -->
        <aside class="fixed top-4 left-4 bottom-4 w-[280px] bg-white/70 backdrop-blur-2xl border border-indigo-100/30 z-50 rounded-[32px] shadow-[0_8px_32px_rgba(99,102,241,0.06)] transition-all duration-500 group">
            <div class="flex flex-col h-full">
                ...
                <div class="h-24 flex items-center px-10">
                    <Link href="/student/dashboard" class="flex items-center gap-3 group/logo">
                        ...
                        <span class="text-2xl font-heading font-extrabold tracking-tight text-stone-800">LMS</span>
                    </Link>
                </div>
                ...
                <nav class="flex-1 px-4 py-6 space-y-1.5">
                    <Link
                        ...
                        :class="[
                            isActive(item.component)
                                ...
                                : 'text-stone-500 hover:text-indigo-600 hover:bg-indigo-50/50 hover:translate-x-1',
                    ...
                    >
                        <component
                            :is="item.icon"
                            :class="[
                                isActive(item.component) ? 'text-white' : 'text-stone-400 group-hover:text-indigo-500',
                ...
                <div class="p-6">
                    <button
                        @click="logout"
                        class="w-full flex items-center px-6 py-3.5 text-sm font-semibold text-stone-400 hover:text-indigo-600 hover:bg-indigo-50/50 rounded-2xl transition-all duration-500 group"
                    >
        ...
        <div class="pl-[312px] pr-8 min-h-screen relative z-10">
            <!-- Header -->
            <header class="h-20 bg-white/50 backdrop-blur-md border border-indigo-100/20 sticky top-4 z-40 px-10 rounded-[24px] shadow-sm flex items-center justify-between mb-8 transition-all duration-500">
                <div class="flex items-center gap-6">
                    <div class="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.2em] text-stone-400">
                        <span class="hover:text-stone-700 cursor-pointer transition-colors">Portal</span>
                        <ChevronRight class="w-3 h-3" />
                        <span class="text-indigo-500 font-semibold">{{ currentPageName }}</span>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-8">
                    <div class="flex items-center gap-4 group cursor-pointer p-1.5 pr-4 rounded-2xl hover:bg-white/80 transition-all duration-500">
                        <div class="relative">
                            ...
                            <div class="absolute -top-1 -right-1 w-3.5 h-3.5 bg-teal-400 border-2 border-white rounded-full"></div>
                        </div>
                        <div class="text-left hidden md:block">
                            <p class="text-xs font-semibold leading-none text-stone-800 group-hover:translate-x-1 transition-transform duration-500">{{ $page.props.auth.user.name }}</p>
                            <p class="text-[9px] text-stone-400 font-medium uppercase tracking-widest mt-1">Student</p>
                        </div>
                    </div>
                </div>
            </header>
